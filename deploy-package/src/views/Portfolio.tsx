...
const PortfolioMedia = ({
  ...
  const resolvedVideo = (coverVideoRef && !coverVideoRef.startsWith("idb://")) ? coverVideoRef : undefined;
...
const PortfolioPage = ({ portfolio: propPortfolio, featured: propFeatured }: PortfolioPageProps) => {
  const { t, lang } = useI18n();
  const [filter, setFilter] = useState<"all" | PortfolioCategory>("all");

  const cases = propPortfolio.length > 0 ? propPortfolio : defaultPortfolio.filter((p) => p.active);
  const featured = propFeatured ?? cases[0] ?? null;
        ...
        <section className="container-x">
          <article className="cinematic-card rounded-[1.5rem] bg-[#111111] text-foreground" data-cinematic>
            <div className="grid lg:grid-cols-12">
              <Link href={`/portfolio/${featured.slug}`} className="group relative min-h-[440px] overflow-hidden lg:col-span-7">
