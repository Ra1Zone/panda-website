...
import type { PortfolioItem, GalleryItem } from "@/store/types";

const GalleryVideo = ({ src, poster }: { src: string; poster?: string }) => {
  const videoRef = useRef<HTMLVideoElement>(null);
  // Only use regular URLs (not idb://)
  const resolvedSrc = (src && !src.startsWith("idb://")) ? src : undefined;
...
const ProjectDetailPage = ({ project: p }: ProjectDetailPageProps) => {
    ...
    <>
      <section className="container-x pt-10 md:pt-14">
        <Button asChild variant="ghost" size="sm">
          <Link href="/portfolio"><ArrowLeft className="rtl:rotate-180" /> {t("nav.portfolio")}</Link>
        </Button>
...
// Separate component so video logic is isolated
const MainMedia = ({
  mediaType,
  coverVideoRef,
  coverImage,
