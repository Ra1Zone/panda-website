...
import { defaultHomeContent, defaultServices, defaultPortfolio, defaultTestimonials, defaultBrands } from "@/store/defaults";
import type { HomeContent, ServiceItem, PortfolioItem, Testimonial, Brand } from "@/store/types";
...
const Hero = ({ home }: { home: HomeContent }) => {
  ...
  return (
    <section className="relative isolate overflow-hidden">
      ...
      <div className="container-x relative z-10 pb-16 pt-20 md:pb-20 md:pt-24 lg:pb-24 lg:pt-28">
        <div className={cn(
          "grid items-center gap-10 lg:gap-10",
          lang === "ar" ? "hero-rtl-layout" : "lg:grid-cols-12 lg:gap-14"
        )}>
          <div className={cn("fade-up", lang === "ar" ? "" : "lg:col-span-6")}>
            ...
            <h1 className={cn(
              "max-w-4xl font-bold tracking-[-0.03em]",
              lang === "ar"
                ? "hero-title-ar text-[clamp(2.4rem,5vw,4.2rem)] leading-[1.25]"
                : "text-[clamp(3rem,6vw,6rem)] leading-[1.1] md:leading-[0.97]"
            )} data-cinematic style={{ "--delay": "90ms" } as CSSProperties}>
            ...
            <p className="mt-6 lede md:mt-8" data-cinematic style={{ "--delay": "160ms" } as CSSProperties}>
              {getLocalized(home.heroSubtitle, lang, t("hero.sub"))}
            </p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap md:mt-10" data-cinematic style={{ "--delay": "230ms" } as CSSProperties}>
              <Button asChild size="lg" variant="ink" className="w-full sm:w-auto">
                <Link href={home.heroPrimaryCtaHref || "/contact"}>
                  {getLocalized(home.heroPrimaryCtaLabel, lang, t("cta.start"))} <ArrowRight className="rtl:rotate-180" />
                </Link>
              </Button>
              <Button asChild size="lg" variant="glass" className="w-full border-primary/20 hover:border-primary/40 sm:w-auto">
                <Link href={home.heroSecondaryCtaHref || "/portfolio"}>
                  {getLocalized(home.heroSecondaryCtaLabel, lang, t("cta.work"))} <ArrowUpRight className="rtl:-scale-x-100" />
                </Link>
              </Button>
            </div>
            {tabs.length > 0 && (
              <div className="hero-strategy-tabs mt-8 grid max-w-xl gap-px overflow-hidden rounded-[1rem] border border-white/10 bg-white/10 shadow-[0_20px_70px_hsl(0_0%_0%/0.28)] md:mt-10" style={{ gridTemplateColumns: `repeat(${Math.min(tabs.length, 3)}, 1fr)`, "--delay": "300ms" } as CSSProperties} data-cinematic>
                {tabs.map((tab) => (
                  <div key={tab.id} className="bg-[#151515]/86 px-3 py-3.5 backdrop-blur transition-colors duration-300 hover:bg-primary/10 sm:p-4">
                    <span className="text-[11px] font-bold text-primary sm:text-xs">{tab.number}</span>
                    <p className="mt-2 text-xs font-semibold sm:mt-3 sm:text-sm">{getLocalized(tab.title, lang)}</p>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className={cn("fade-up", lang === "ar" ? "" : "lg:col-span-6")} style={{ animationDelay: "0.2s" }}>
            <div className="hero-media-shell home-panel float-soft relative overflow-hidden rounded-[1.75rem] p-4 sm:p-5 hero-media-desktop" data-cinematic style={{ "--delay": "180ms" } as CSSProperties}>
              ...
              <div className="hero-collage relative grid h-full grid-cols-12 grid-rows-12 gap-3 sm:gap-4">
                ...
                <div className="col-span-5 row-span-5 overflow-hidden rounded-[1.25rem] bg-[#1A1A1A] shadow-[0_18px_48px_hsl(0_0%_0%/0.34)] hero-collage-bottom-img">
                  <img src={home.heroBottomImage || homeAssets.brandWall} alt="" className="h-full w-full object-cover" />
                </div>
                <div className="col-span-7 row-span-5 rounded-[1.25rem] bg-[#0B0B0B] p-4 text-foreground shadow-[0_22px_70px_hsl(0_0%_0%/0.42)] sm:p-5">
                  ...
                  <div className="mt-5 space-y-2.5 sm:mt-8 sm:space-y-3">
                    {campaignItems.map((item) => (
                      <div key={item.id} className="rounded-[0.9rem] border border-white/10 bg-white/[0.06] p-2.5 sm:p-3">
                        <div className="flex items-center gap-2.5 sm:gap-3">
                          <span className="flex size-7 shrink-0 items-center justify-center rounded-full bg-primary text-[10px] font-bold text-primary-foreground sm:size-8 sm:text-[11px]">{item.number}</span>
                          <span className="text-xs font-semibold sm:text-sm">{getLocalized(item.title, lang)}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                </div>
                <div className="absolute bottom-4 rounded-full border border-white/10 bg-[#151515]/88 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.18em] text-foreground shadow-sm backdrop-blur ltr:left-4 rtl:right-4 sm:bottom-5 sm:px-4 sm:py-2 sm:text-xs sm:tracking-[0.2em] ltr:sm:left-5 rtl:sm:right-5">
                  {getLocalized(home.heroBottomBarText, lang, t("hero.bottombar"))}
                </div>
                <div className="absolute rounded-[0.875rem] border border-primary/20 bg-[#101510]/85 px-3 py-2.5 shadow-[0_18px_50px_hsl(var(--primary)/0.12)] backdrop-blur ltr:right-4 ltr:top-4 rtl:left-4 rtl:top-4 sm:rounded-[1rem] sm:px-4 sm:py-3 ltr:sm:right-5 ltr:sm:top-5 rtl:sm:left-5 rtl:sm:top-5">
                  <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-primary sm:text-[10px] sm:tracking-[0.22em]">
...
const Clients = ({ brands }: { brands: Brand[] }) => {
  const clients = brands.map((b) => ({ name: b.name, logo: b.logo, category: b.category, story: b.story }));
...
const Services = ({ home, services }: { home: HomeContent; services: ServiceItem[] }) => {
          ...
          <div className="flex lg:col-span-5 lg:items-end lg:pt-2">
            <div className="max-w-md">
              ...
              <Button asChild variant="outline" size="lg" className="mt-8">
                <Link href="/services">{t("services.cta.explore")} <ArrowRight className="rtl:rotate-180" /></Link>
              </Button>
            </div>
          </div>
        </div>
...
const PortfolioCardMedia = ({ p, aspectClass }: { p: PortfolioItem; aspectClass: string }) => {
  ...
  // Only use regular URLs (not idb://)
  const resolvedVideo = (mediaType === "video" && coverVideoRef && !coverVideoRef.startsWith("idb://")) ? coverVideoRef : undefined;
...
const PortfolioPreview = ({ portfolio }: { portfolio: PortfolioItem[] }) => {
  const { t, lang } = useI18n();
      ...
      <div className="container-x relative">
        <div className="green-thread mb-14 flex flex-col gap-8 md:mb-16 md:flex-row md:items-end md:justify-between" data-cinematic>
          ...
          <Button asChild variant="glass" size="lg" className="self-start text-foreground md:self-auto">
            <Link href="/portfolio">{t("portfolio.view")} <ArrowRight className="rtl:rotate-180" /></Link>
          </Button>
        </div>
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-12">
          {projects.map((p, i) => (
            <Link key={p.id} href={`/portfolio/${p.slug}`} className={cn("group block", i === 0 || i === 3 ? "lg:col-span-7" : "lg:col-span-5")} data-cinematic style={{ "--delay": `${i * 80}ms` } as CSSProperties}>
        ...
        >
          <CarouselContent className="-ml-4 md:-ml-6">
            {items.map((item, slideIndex) => (
              <CarouselItem key={item.id} className="pl-4 md:pl-6">
                <figure className="testimonial-card relative overflow-hidden rounded-[1.25rem] border border-white/10 bg-[#151515]/82 p-5 shadow-[0_28px_100px_hsl(0_0%_0%/0.44)] backdrop-blur sm:rounded-[1.5rem] sm:p-6 md:p-9 lg:p-10">
...
const FinalCTA = ({ home }: { home: HomeContent }) => {
  const { t, lang } = useI18n();
      ...
      <div className="container-x relative text-center">
        ...
        <div className="mt-12">
          <Button asChild size="lg" className="bg-primary text-primary-foreground hover:bg-primary/90">
            <Link href="/contact">{t("cta.start")} <ArrowRight className="rtl:rotate-180" /></Link>
          </Button>
        </div>
      </div>
    </section>
...
const IndexPage = ({ home, services, portfolio, testimonials, brands }: IndexPageProps) => {
  const resolvedHome = home ?? defaultHomeContent;
  const resolvedServices = services.length > 0 ? services : defaultServices;
  const resolvedPortfolio = portfolio.length > 0 ? portfolio : defaultPortfolio.filter((p) => p.active);
  const resolvedTestimonials = testimonials.length > 0 ? testimonials : defaultTestimonials.filter((t) => t.active);
  const resolvedBrands = brands.length > 0 ? brands : defaultBrands;

  return (
    <div className="home-surface home-surface--home">
      <Hero home={resolvedHome} />
      <BrandManifesto />
      <Clients brands={resolvedBrands} />
      <Services home={resolvedHome} services={resolvedServices} />
      <PortfolioPreview portfolio={resolvedPortfolio} />
      <Process />
      <Why />
      <Testimonials testimonials={resolvedTestimonials} />
      <FinalCTA home={resolvedHome} />
    </div>
  );
};
