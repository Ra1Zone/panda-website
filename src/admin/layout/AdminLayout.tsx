...
export const AdminLayout = () => {
    ...
    <div className="flex h-full flex-col">
      ...
      <nav className="flex-1 space-y-0.5 overflow-y-auto px-3 py-4">
        {NAV.map(({ to, icon: Icon, label }) => (
          <NavLink key={to} to={to} onClick={() => setSidebarOpen(false)} end={to === "/admin"}>
            {({ isActive }) => (
              <span className={cn(
  ...
  return (
    ...
    <div dir="ltr" className="flex h-screen overflow-hidden bg-[#0A0A0A] text-white">
      {/* Desktop sidebar */}
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
        ...
        <header className="flex h-14 shrink-0 items-center gap-4 border-b border-white/8 bg-[#0D0D0D] px-5">
          ...
          <Link to="/" target="_blank" className="ml-auto text-xs text-white/35 hover:text-primary">
            View site ↗
          </Link>
        </header>
