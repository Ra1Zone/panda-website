...
import { testimonialsStore } from "@/store/dataStore";
...
const TestimonialsManager = () => {
  const [items, setItems] = useState<Testimonial[]>(() => testimonialsStore.getAll());
  const [editing, setEditing] = useState<Testimonial | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);

  const persist = (data: Testimonial[]) => { testimonialsStore.save(data); setItems(data); };
  ...
  const saveEdit = async () => {
    ...
    persist(exists ? items.map((t) => (t.id === editing.id ? editing : t)) : [...items, editing]);
    setSaving(false);
    toast.success("Testimonial saved!");
    cancelEdit();
  };

  const remove = (id: string) => { persist(items.filter((t) => t.id !== id)); toast.success("Deleted."); };
