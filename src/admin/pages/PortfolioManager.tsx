...
const PortfolioManager = () => {
  const [items, setItems] = useState<PortfolioItem[]>(() => portfolioStore.getAll());
  ...
  const persist = (data: PortfolioItem[]) => {
  ...
  const saveEdit = async () => {
    ...
    persist(exists ? items.map((p) => (p.id === record.id ? record : p)) : [...items, record]);
    setSaving(false);
    toast.success("Project saved!");
    cancelEdit();
  };

  const remove = (id: string) => { persist(items.filter((p) => p.id !== id)); toast.success("Project deleted."); };
